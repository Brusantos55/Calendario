...
function showInfo(indice, i, ini){
  ...
  document.querySelector(".info").innerHTML= "<div class='card'><h2>"
    +actividad[indice]['nombre']+"</h2><img src='../../reto/img/"
    +actividad[indice]['foto']+"' width='100%' /><p>"
    +actividad[indice]['observaciones']+"</p><p class='fechalugaractiviades'>"
    +actividad[indice]['sFecha']+" || "+actividad[indice]['sHora']+" || "
    +actividad[indice]['Lugar']+"</p><div class='btn-container'><form action='../../reto/1.inscripccion.php' method='post'><input name='actID' type='hidden' value='"
    +actividad[indice]['ID']+"'/><input name='inscribirse' type='submit' value='Inscribirse'/></form></div></div>";
}
